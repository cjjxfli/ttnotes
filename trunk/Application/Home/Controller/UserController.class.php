...
class UserController extends HomeController{
    ...
    public function register(){
        if(IS_POST){
            ...
            $user_email = "";
            $data_insert = array();
            $user_name = I("post.user_email");
            $user_password = I("post.email_password");
            $user_repeat_password = I("post.email_repeat_password");
            $user_verify = I("post.email_verify");
            $user_email = $user_name;
            $data_insert["user_email"] = $user_email;
            $data_insert["user_login"] = $user_email;
            $data_insert["user_status"] = 1;

            $user_model = D("Users");
            if(empty($user_name) || empty($user_password) || empty($user_repeat_password)
                || empty($user_verify)){
            ...
            if($user_model->check_user($user_email)){
                $data_json = array("flag" => 0,"msg" => "用户已存在!");
                $this->ajaxReturn($data_json);
            }
            if($user_password != $user_repeat_password){
                $data_json = array("flag" => 0,"msg" => "两次输入的密码不一样,请重新输入!");
                $this->ajaxReturn($data_json);
            }
            ...
            $subject = "邮箱激活";
            $regtime = time();
            $token = md5($user_name.$user_password.$regtime); //创建用于激活识别码
            $token_exptime = $regtime + 60*60*24*1000;//过期时间为24小时后
            $data_insert["email_token"] = $token;
            $data_insert["status"] = 0;
            $data_insert["email_send_time"] = $regtime;
            $data_insert["email_expired_time"] = $token_exptime;

            $email_body = email_body($user_name,$subject,$token,1);
            $send_statue = $this->send_email($user_email,"邮箱激活",$email_body);
            if(is_bool($send_statue) && !$send_statue){
                $data_json = array("flag" => 0,"msg" => $send_statue);
                $this->ajaxReturn($data_json);
            }
            $data_insert["user_pass"] = $user_password;
            $data_insert["create_time"] = date("Y-m-d H:m:s",time());
            $user_model->add_user_by_array($data_insert);

            $data_json = array("flag" => 1,"msg" => "注册成功,请登录!");
            $this->ajaxReturn($data_json);
        }
        $this->display();
    }
